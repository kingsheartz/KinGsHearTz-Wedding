...
<template>
    ...
    -->
		...
		<div class="flex flex-wrap items-center justify-center gap-2 sm:gap-3 mb-8 max-w-3xl mx-auto">
			<button v-for="(loc, i) in locations" :key="loc.id" type="button"
				class="rounded-full px-4 py-2 text-left transition border text-xs sm:text-sm min-w-0 sm:min-w-[140px]" :class="activeIndex === i
		...
		<div
			class="max-w-4xl mx-auto rounded-2xl overflow-hidden border border-zinc-700/80 shadow-[0_24px_60px_rgba(0,0,0,0.45)] bg-zinc-900">
			<iframe :key="active.id" title="Wedding location map" width="100%" height="400"
				class="w-full min-h-[320px] sm:min-h-[400px] border-0" loading="lazy"
				referrerpolicy="no-referrer-when-downgrade" :src="active.embed" />
		</div>
	</section>
</template>
